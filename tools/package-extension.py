...
The native library ships bundled INSIDE the extension under
    src/code/<arch>-<platform>/coinxt.{so,dll,dylib}
with the bare token name (no "lib" prefix). Installing the packaged extension
then lets the engine resolve "c:coinxt>" via the revLibraryMapping with no
loose library, no sudo, no LD_LIBRARY_PATH. (The SodiumXT / TorrentXT model;
see CLAUDE.md.)
...
tests the full platform matrix and refreshes all platforms on merge to main;
this script handles the one platform you are on.

Two manifests, two jobs (do not merge them):
    native/MANIFEST.sha256    pins the vendored trezor-crypto SOURCES;
    src/code/MANIFEST.sha256  pins the committed native BINARIES (this script).

The platform ids are architecture-FIRST and Windows is "-win32" for BOTH
bitnesses (the family convention):
...
# The provenance manifest recording the SHA256 of every committed native blob
# under src/code/. It lets a consumer (and the CI verify-binaries job) confirm
# a committed binary was not silently swapped or corrupted relative to what the
# packaging step recorded. It does NOT by itself prove a blob matches the
# source it was built from - that guarantee comes from CI rebuilding every
# platform from the pinned vendored sources on merge to main. Format is plain
# `sha256sum` (hash, two spaces, path relative to src/code), so it verifies
# with `cd src/code && sha256sum -c MANIFEST.sha256`.
...
def default_platform_id():
    os_token = detect_os()
    if os_token == "mac":
        # macOS ships one universal (arm64 + x86_64) binary; a locally built
        # dylib may be single-arch, so a true universal build is a CI concern.
        # It is still filed under the canonical universal-mac id.
...
def write_manifest(repo_root):
    ...
    Scans src/code/<id>/coinxt.{so,dll,dylib}, records each blob's SHA256, and
    ...
    for platform_id in sorted(os.listdir(code_dir)):
        ...
        for suffix in PLATFORM_SUFFIX.values():
            blob = os.path.join(blob_dir, f"{TOKEN}{suffix}")
            if os.path.isfile(blob):
                rel = os.path.join(platform_id, f"{TOKEN}{suffix}")
...
def main(argv):
    ...
    dest = os.path.join(dest_dir, f"{TOKEN}{suffix}")
    ...
    write_manifest(args.repo_root)
    print("Remember to `git add` the refreshed binary AND src/code/MANIFEST.sha256 "
          "in the SAME change as the native edit (CLAUDE.md workflow rule).")
    return 0
